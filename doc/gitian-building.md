...
*Setup instructions for a Gitian build of MCAN Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the MCAN
...
- [Building MCAN Core](#building-MCAN-core)
...
Clone the git repositories for MCAN and Gitian.
...
in the MCAN repository under 'Fetch and create inputs' to install sources which require
...
To build MCAN Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the MCAN repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/MCAN/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/MCAN-project/MCAN
...
For example:
```bash
URL=https://github.com/thrasher-/MCAN.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit MCAN=${COMMIT} --url MCAN=${URL} ../MCAN/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit MCAN=${COMMIT} --url MCAN=${URL} ../MCAN/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit MCAN=${COMMIT} --url MCAN=${URL} ../MCAN/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the MCAN git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../MCAN/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/MCAN
SIGPATH=/some/root/path/MCAN-detached-sigs

./bin/gbuild --url MCAN=${BTCPATH},signature=${SIGPATH} ../MCAN/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/MCAN-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/MCAN-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/MCAN-osx-build.assert
```
...
[MCAN-project/gitian.sigs.MCAN](https://github.com/MCAN-project/gitian.sigs.MCAN/) repository, or if that's not possible create a pull
